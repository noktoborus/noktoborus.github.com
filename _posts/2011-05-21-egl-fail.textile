...
Начнём с самого простого: напишем маленькую програмку для определения нашей версии EGL.
...
h3. Описание атрибутов EGLConfig
...
| EGL_CONFIG_ID				| integer	|=. @----@				|=. 					| ID EGLConfig, относительно которого создаётся эта поверхность.	|
| EGL_HEIGHT				| integer	|=. @+---@				|=.						| Высота поверхности.	|
| EGL_WIDTH					| integer	|=. @+---@				|=. 					| Ширина поверхности.	|
| EGL_HORIZONTAL_RESOLUTION	| integer	|=. @----@				|=. 					| Горизонтальный шаг точки.	|
| EGL_VERTICAL_RESOLUTION	| integer	|=. @----@				|=. 					| Вертикальный шаг точки.	|
| EGL_LARGEST_PBUFFER		| boolean	|=. @+---@				|=. 					| TODO: If true, create largest pbuffer possible.	|
| EGL_MIPMAP_TEXTURE		| boolean	|=. @+--+@				|=. 					| @EGL_TRUE@, если текстура поддерживает множественные отображения (mipmaps).	|
...
| EGL_PIXEL_ASPECT_RATIO	| integer	|=. @----@				|=. 					| Отобразить соотношение сторон.	|
| EGL_MULTISAMPLE_RESOLVE	| enum		|=. @----@				|=. 					| TODO: multisample resolve behavior.	|
| EGL_RENDER_BUFFER			| enum		|=. @--+-@				|=. 					| Буфер рендера.	|
| EGL_SWAP_BEHAVIOR			| enum		|=. @----@				|=. 					| TODO: buffer swap behavior.	|
| EGL_TEXTURE_FORMAT		| enum		|=. @+--+@				|=. 					| Формат текстуры: RGB, RGBA или никакой.	|
| EGL_TEXTURE_TARGET		| enum		|=. @+--+@				|=. 					| Тип текстуры: 2D или нет текстуры.	|
| EGL_VG_ALPHA_FORMAT[7]	| enum		|=. @++++@				|=. 					| Формат прозрачности для OpenVG.	|
| EGL_VG_COLORSPACE[7]		| enum		|=. @++++@				|=. 					| Пространство цвета для OpenVG.	|
...
fn7. TODO: attributes of the surface are determined by the VGImageFormat of buffer.
